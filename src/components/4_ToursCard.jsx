...
const ToursCard = ({tour, d, people, guides, stay, difficulty, price}) => {
    ...
    return(
        <div className="flex flex-col items-center justify-center w-[300px] sm:w-[350px] md:w-[260px] lg:w-[330px] gap-y-1 text-[#777777] rounded-sm shadow-lg">
            <div className={`relative w-full h-[185px] bg-cover bg-center bg-no-repeat bg-blend-screen clip-path-sm-1
                ...
                <span className={`absolute bottom-1/3 right-0 font-light text-[22.4px] text-white text-right bg-opacity-70 px-3 py-2 ${d === 3 ? 'bg-amber-600' : d === 7 ? 'bg-green-500' : 'bg-blue-600'}`}>{tour.toUpperCase()}</span>
            </div>
            ...
            <div className={
                ...
                <a href='/' className='bg-[white] px-9 py-5 rounded-full text-xs md:text-sm lg:text-base cursor-pointer transition-all hover:animate-bounce'>
                BOOK NOW!
                </a>
            </div>
        </div>
